...
public class editAccount {
    ...
    public Mono<RedirectView> editPassword(Model model,@Valid @ModelAttribute PasswordUdateForm form, BindingResult result, WebSession session) {
        Map<String, Object> flashAttributes = new HashMap<>();
        if (result.hasErrors()) {
        ...
        return signupApi.editPassword(form).then(Mono.just(new RedirectView("/bankapp", HttpStatusCode.valueOf(301))));
    }
