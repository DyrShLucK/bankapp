...
public class ApiController implements DefaultApi {
    ...
    @Override
    public Mono<ResponseEntity<TransferResponse>> apiTransferPost(@jakarta.annotation.Nullable String userName, Mono<Transfer> transfer, ServerWebExchange exchange) {
        if (userName == null) {
            return Mono.just(ResponseEntity.status(HttpStatus.UNAUTHORIZED).build());
        }
        return apiService.getTransferResponse(transfer, userName, userName).map(ResponseEntity::ok);
    }
