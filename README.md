...
### Взаимодействие между микросервисами осуществляется через Apache Kafka вместо REST
- Используются следующие топики:
- - notifications.requests — для отправки запросов на создание уведомлений
- - notifications.responses — для получения ответов от других сервисов, требующих уведомлений
- - exchange.rates — для передачи актуальных курсов валют от Exchange Generator Service к Exchange Service
- Микросервис Notifications реализует стратегию доставки at-least-once:
- - Отключён автоматический коммит offset'ов
- - Подтверждение (acknowledge) отправляется только после успешной обработки сообщения
- - При перезапуске сервис продолжает обработку с последнего подтверждённого offset
- Микросервисы Exchange и Exchange Generator реализуют стратегию доставки at-most-once:
- - Используется автоматический коммит offset'ов
- -  Гарантируется упорядоченность сообщений за счёт отправки всех данных с одинаковым ключом в одну партицию
